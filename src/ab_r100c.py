#!/usr/bin/env python3
"""R100 positive-test hunt: my-bot (user-proxy) at NARROW K9/K11 where R100 can fire."""
...
def matrix(botL, opp, NP, KP, seeds, tag):
    ...
    for i,s in enumerate(seeds):
        ...
        res,reason=run_game(L,R,s,NP,KP,HERE/f"_abc_{tag}.log")
        if res=="ERR": continue
...
def main():
    ...
    K={9:(26,4),11:(33,5)}; s12=list(range(2000,2012))
    ON="proto-bot.py"; OFF="proto_r100off.py"
    for kk in (9,11):
        NP,KP=K[kk]
        wN,lN,dN,cN=matrix(ON,"my-bot.py",NP,KP,s12,"on")
        wF,lF,dF,cF=matrix(OFF,"my-bot.py",NP,KP,s12,"off")
        tag=""
        ...
        print(f"  my-bot.py K{kk:2d}: ON {wN}W{lN}L{dN}D cr{cN} | OFF {wF}W{lF}L{dF}D cr{cF}{tag}"); sys.stdout.flush()
    print("AB_R100C_DONE")
main()
