...
# ============================================================================
# BASE-EATER test opponent: reproduces the real games 7/8 econ-swarmer -- build
# an economy, mass a ~WAVE_MIN stack, then GRIND the enemy's FORWARD BASES one
# by one (NOT a HQ beeline). When the stack is worn down, retreat home to
# regroup, then push again. This is the one behaviour no existing sparring bot
# has, and the exact one that exercises proto's FORWARD-PUSH/base_eating/relief
# defensive gates. Pure test harness; efficiency is irrelevant.
# ============================================================================
DEBUG = False
ECON_BASES = 2        # claim/build this many near strongholds for income
ARMY_CAP = 26         # train up to this many warriors
WAVE_MIN = 12         # form the grinding stack once the mobile column reaches this
REGROUP_AT = 5        # if the pushing stack falls below this, retreat home to regroup
GOLD_BUF = 40
...
class Bot:
    ...
    econ: list = field(default_factory=list)      # strongholds we use for economy
    marching: bool = False
    tgt: int = -1                                  # current enemy base being ground
...
def decide(S, M, nav, turn):
    ...
    try:
        ...
        if DEBUG: print(f"# baseeater err t{turn}: {e}", file=sys.stderr, flush=True)
    return a
...
def _impl(S, M, nav, turn, a):
    ...
    if not BOT.inited:
        ...
        BOT.econ = BOT.claim_order[:ECON_BASES]
        BOT.inited = True
    ...
    my_regs = {b.region for b in S.buildings if b.side is me}
    ...
    # --- ECONOMY: build bases on our near strongholds; keep 1 worker each -----
    workers = set()
    for reg in BOT.econ:
        b = S.find_building(reg)
        onit = next((w for w in my if w.region == reg and w.state is WState.STATIONARY), None)
        if b is None and reg not in enemy_at:
            if onit is not None:
                if S.gold - spent - BASE_LEVELS[1].cost >= 0:
                    spent += BASE_LEVELS[1].cost; a.upgrades.append(reg); workers.add(onit.id)
            elif not any(w.state is WState.MOVING and w.target == reg for w in my):
                cand = next((w for w in my if w.state is WState.STATIONARY and w.region == M.my_hq), None)
                if cand is not None: mv(cand, reg); workers.add(cand.id)
        elif b is not None and onit is not None:
            workers.add(onit.id)   # keep one worker sitting on each base for income

    # --- ARMY: everyone not a base worker is the mobile column ---------------
    column = [w for w in my if w.id not in workers
              and not (w.state is WState.MOVING and w.target in BOT.econ)]
    stat = [w for w in column if w.state is WState.STATIONARY]
    if len(column) >= WAVE_MIN:
        BOT.marching = True

    # target = the enemy (proto) BASE nearest our stack; grind forward bases first.
    enemy_bases = [b.region for b in S.buildings if b.side is not me and b.type is BType.BASE]
    if BOT.marching and column:
        cnt = Counter(w.region for w in column)
        stack = cnt.most_common(1)[0][0]
        at_stack = [w for w in stat if w.region == stack]
        # regroup home if the pushing stack is worn down
        if len(column) < REGROUP_AT:
            BOT.marching = False
            for w in stat:
                if w.region != M.my_hq: mv(w, M.my_hq)
        else:
            # (re)pick the nearest still-enemy base to the stack; fall back to enemy HQ
            if BOT.tgt < 0 or BOT.tgt not in enemy_bases:
                BOT.tgt = min(enemy_bases, key=lambda r: nav.hops(stack, r)) if enemy_bases else M.opp_hq
            tgt = BOT.tgt
            together = len(at_stack) >= max(2, int(0.6 * len(column)))
            if stack == tgt:
                for w in stat:
                    if w.region != stack: mv(w, stack)     # pile on, siege it
            elif together:
                nh = nav.next_hop(stack, tgt)
                for w in at_stack:
                    if nh >= 0: mv(w, nh)                   # advance the whole stack one hop
                for w in stat:
                    if w.region != stack: mv(w, stack)
            else:
                for w in stat:
                    if w.region != stack: mv(w, stack)     # regroup the stack first
    else:
        for w in stat:                                     # gather at home until WAVE_MIN
            if w.region != M.my_hq and w.region not in BOT.econ: mv(w, M.my_hq)

    # --- TRAIN up to the army cap -------------------------------------------
    if hq and len(my) < ARMY_CAP + ECON_BASES:
        cap = HQ_LEVELS[hq.level].train_cap; n = cap
        while n > 0 and not can(TRAIN_COST * n): n -= 1
        a.train_n = n
    # occasionally climb the HQ so training capacity + hp grow (keeps it realistic)
    if hq and hq.level < 3 and turn > 40 and S.gold - spent - hq.upgrade_cost() >= 300 and M.my_hq not in enemy_at:
        spent += hq.upgrade_cost(); a.upgrades.append(M.my_hq)
