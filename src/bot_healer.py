...
def _decide_impl(S: GameState, M: GameMap, nav: Nav, turn: int, a: Actions) -> None:
    ...
    # 1a-bis) HEAL-WALL (proxy behaviour): aggressively repair EVERY damaged base so a
    #     trickle-chipping enemy can never convert. Reproduces the game-4 opponent that
    #     healed its bases 23x. UPGRADE on a max base = heal-to-full (referee); on a
    #     non-max base it upgrades (which also heals). Spend down to the upkeep cushion.
    for b in sorted(my_bases, key=lambda bb: bb.region):
        if b.region in upgraded_regions or not upgrade_legal(b.region):
            continue
        if b.hp < b.current_hp():
            cost = BASE_HEAL_COST if _is_max(b) else _next_cost(b)
            if can(cost, ignore_heal_reserve=True):
    ...
    if (hq is not None and hq.region not in upgraded_regions and not _is_max(hq)
        ...
        # HEAL-WALL proxy CLIMBS HARD to L5 (the game-4 analog: a strong econ that ALSO
        # reaches the HQ cap, so the tiebreak is decided by who breaks the other's HQ --
        # and the aggressive base-heal makes a trickle-chipper unable to do so).
        want = False
        if hq.level < 3:
            want = econ_ok
        else:
            want = (turn >= 30) and (S.gold - spent) >= (cost + reserve)
