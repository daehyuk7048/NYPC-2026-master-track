...
import os
DEBUG = False
DBGPATH = os.environ.get("ALLIN_DBG", "")   # set this env var to a path to log per-turn state (diagnostic only)
GOLD_FLOOR = 40
ECON_BASES = 5         # build a REAL economy (match the defender) to fund a sustained army
BASE_UP_LEVEL = 2      # upgrade bases to L2 (work_cap 2) -> more income -> bigger army
HQ_LEVEL_CAP = 2       # keep the HQ LOW (L2: 15hp, survives a counter) but pour everything else into army
COMMIT_ARMY = 8        # start pushing once the army reaches this -> sustained pressure
ECON_DONE_TURN = 70    # by here the economy should be up; afterwards train army flat-out
...
def decide(S: GameState, M: GameMap, nav: Nav, turn: int) -> Actions:
    ...
    try:
        ...
        if DEBUG:
            print(f"# allin err t{turn}: {e}", file=sys.stderr, flush=True)
    if DBGPATH and turn % 10 == 0:
        try:
            me = M.my_side
            myarmy = sum(1 for w in S.warriors if w.id.side is me)
            mybases = sum(1 for b in S.buildings if b.side is me and b.type is BType.BASE)
            ehq = S.find_building(M.opp_hq); ehl = ehq.level if ehq else 0
            ehp = ehq.hp if ehq else 0
            ebases = sum(1 for b in S.buildings if b.side is not me and b.type is BType.BASE)
            with open(DBGPATH, "a") as _f:
                _f.write(f"t{turn} {me.value} myArmy={myarmy} myBases={mybases} myGold={S.gold} "
                         f"| DEF_HQ=L{ehl}/{ehp} DEF_bases={ebases}\n")
        except Exception:
            pass
    return a
...
def _decide_impl(S, M, nav, turn, a):
    ...
    # ===== ECONOMY: build a real base economy, keep the HQ LOW, everything else -> army =====
    # build a base on any claimer sitting on an unowned stronghold
    for w in my:
        if (w.state is WState.STATIONARY and w.region in BOT.claim_set
                ...
                and w.region not in enemy_at and len(my_bases) < ECON_BASES):
            if can(BASE_LEVELS[1].cost):
                up(w.region, BASE_LEVELS[1].cost)
    # upgrade bases toward BASE_UP_LEVEL (work_cap -> income -> army)
    for b in sorted(my_bases, key=lambda b: b.level):
        if b.level < BASE_UP_LEVEL and ulegal(b.region) and b.region not in ups:
            c = _next_cost(b)
            if can(c):
                up(b.region, c)
    # climb the HQ ONLY up to HQ_LEVEL_CAP (the all-in deliberately stays low)
    if hq and hq.level < HQ_LEVEL_CAP and ulegal(hq.region) and hq.region not in ups:
        c = _next_cost(hq)
        if can(c):
            up(hq.region, c)
    # heal the HQ if it is being chipped (stay alive for the tiebreak race)
    if hq and hq.hp < hq.current_hp() and ulegal(hq.region) and hq.region not in ups:
        c = _next_cost(hq)
        if can(c):
            up(hq.region, c)

    # ===== WORKERS: keep work_cap bodies on each base + 1 on the HQ for income =====
    ...
    for r, c in need.items():
        for w in at.get(r, [])[:c]:
            workers.add(w.id)
    free = [w for w in stat if w.id not in workers]

    # ===== CLAIM more strongholds until we have ECON_BASES (economy first) =====
    claimer_ids = set()
    if len(my_bases) < ECON_BASES:
        unclaimed = [s for s in BOT.claim_order
                     if S.find_building(s) is None and s not in enemy_at]
        want = ECON_BASES - len(my_bases)
        for s in unclaimed[:want]:
            cand = [w for w in free if w.id not in assigned]
            if not cand:
                break
            w = min(cand, key=lambda w: nav.hops(w.region, s))
            mv(w, s)
            claimer_ids.add(w.id)

    # ===== ARMY: relentless pressure. Grind the enemy's forward bases, then the HQ =====
    army = [w for w in free if w.id not in claimer_ids]
    ebases = sorted([b.region for b in S.buildings if b.side is not me and b.type is BType.BASE],
                    key=lambda r: nav.hops(M.my_hq, r))
    target = next((r for r in ebases + [M.opp_hq] if S.find_building(r) is not None), M.opp_hq)
    # total army potential (stationary + already-moving non-workers): commit the moment it is big enough
    army_potential = len(my) - len(workers) - len(claimer_ids)
    if army_potential >= COMMIT_ARMY:
        for w in army:
            mv(w, target)                     # everyone funnels onto the target -> a self-stacking siege
    else:
        # too small yet: stage at our base nearest the enemy so the push starts forward, not from home
        stage = M.my_hq
        if my_bases:
            stage = min((b.region for b in my_bases), key=lambda r: nav.hops(r, M.opp_hq))
        for w in army:
            if w.region != stage:
                mv(w, stage)

    # ===== TRAIN: economy FIRST, then army flat-out =====
    # While the economy is still coming up (fewer than ECON_BASES, or bases not yet upgraded, and early),
    # RESERVE gold for the next base build/upgrade so claimers can actually build -- otherwise training
    # drains us to ~60g and claimers sit on strongholds unable to pay the 300 (the bug that starved us).
    econ_done = (len(my_bases) >= ECON_BASES
                 and all(b.level >= BASE_UP_LEVEL for b in my_bases)) or turn >= ECON_DONE_TURN
    econ_reserve = 0 if econ_done else (BASE_BUILD_COST + 200)
    if hq:
        cap = _train_cap(hq)
        n = cap
        while n > 0 and not can(TRAIN_COST * n + econ_reserve):
            n -= 1
        a.train_n = n
