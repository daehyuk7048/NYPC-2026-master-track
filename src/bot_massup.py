...
DEBUG = False
LAUNCH_TURN = 60     # camp the army on our HQ until this turn, then all-in beeline the enemy HQ
N_BASES = 3          # economy bases funding the mass-train
HQ_TGT = 3           # climb HQ to this level (train_cap 2 + 20hp warriors) to mass-train fast
GOLD_BUF = 30


@dataclass
class Bot:
    inited: bool = False; bases: list = field(default_factory=list)
    claim_order: list = field(default_factory=list); marching: bool = False
...
def decide(S, M, nav, turn):
    ...
    try:
        ...
        if DEBUG: print(f"# massup err t{turn}: {e}", file=sys.stderr, flush=True)
    return a
...
def _impl(S, M, nav, turn, a):
    me = M.my_side
    if not BOT.inited:
        BOT.claim_order = sorted(M.strongholds, key=lambda s: nav.hops(s, M.my_hq))[:N_BASES]
        BOT.inited = True
    my = [w for w in S.warriors if w.id.side is me]
    hq = S.find_building(M.my_hq)
    target = M.opp_hq
    spent = 0; assigned = set()
    ...
    my_base_regs = [b.region for b in S.buildings if b.side is me and b.type is BType.BASE]
    launched = turn >= LAUNCH_TURN
    if launched: BOT.marching = True

    # ECONOMY (pre-launch): claim N_BASES nearest strongholds; build when a worker stands on one;
    # keep exactly one worker per base. All OTHER warriors camp on the HQ.
    workers = set()
    if not BOT.marching:
        for s in BOT.claim_order:
            if s in enemy_at: continue
            b = S.find_building(s)
            if b is None:
                onsite = next((w for w in my if w.region == s and w.id not in assigned), None)
                if onsite is not None:
                    if S.gold - spent - BASE_LEVELS[1].cost >= 0:
                        spent += BASE_LEVELS[1].cost; a.upgrades.append(s); workers.add(onsite.id)
                elif not any(w.state is WState.MOVING and w.target == s for w in my):
                    cand = next((w for w in my if w.state is WState.STATIONARY and w.region == M.my_hq
                                 and w.id not in assigned), None)
                    if cand is not None: mv(cand, s); workers.add(cand.id)
            else:
                keeper = next((w for w in my if w.region == s and w.id not in assigned), None)
                if keeper is not None: workers.add(keeper.id)
        # HQ fortify to HQ_TGT for train_cap + warrior hp
        if hq is not None and hq.level < HQ_TGT and hq.region not in a.upgrades:
            c = hq.upgrade_cost()
            if can(c): spent += c; a.upgrades.append(hq.region)

    # ARMY: camp non-workers on the HQ until launch; then march the whole army (workers too) as a
    # cohesive stack at the enemy HQ.
    army = [w for w in my if w.id not in workers]
    if BOT.marching:
        col = [w for w in my if w.state is WState.STATIONARY]   # all-in: everyone
        if col:
            cnt = Counter(w.region for w in col); stack = cnt.most_common(1)[0][0]
            at = [w for w in col if w.region == stack]
            together = len(at) >= max(2, int(0.6 * len(col)))
            if stack == target:
                for w in col:
        ...
        for w in army:
            if w.state is WState.STATIONARY and w.region != M.my_hq: mv(w, M.my_hq)

    # TRAIN every turn at full cap (dump gold into the deathball)
    if hq is not None:
