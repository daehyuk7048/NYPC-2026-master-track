...
def _decide_impl(S: GameState, M: GameMap, nav: Nav, turn: int, a: Actions) -> None:
    ...
    if (hq is not None and hq.region not in upgraded_regions and not _is_max(hq)
        ...
        # G6-B EMULATOR: economic L1->L3 early, then a steady INCOME-GATED climb to L5.
        # The affordability gate is the whole point: if the opponent razes our economy,
        # income drops, gold banks slower, and this climb SLIPS (or never completes) --
        # exactly the mechanism the rebalance experiment is trying to trigger.
        want = False
        if hq.level < 3:
            want = econ_ok                                          # early economic climb
        elif hq.level == 3:
            want = (turn >= 90) and (S.gold - spent) >= (cost + reserve)   # L3->L4 mid-late
        else:  # level 4 -> 5 (the decisive banked climb; real B did this at t161->176)
            want = (turn >= 120) and (S.gold - spent) >= (cost + reserve)  # L4->L5 late
    ...
    # 2c) G6-B EMULATOR: NEVER assault -- hold the whole army HOME near the HQ cluster so it
    #     CONVERGES to defend any base the enemy tries to raze (this is what makes real B's
    #     economy un-crackable: our _can_crack counts this converging garrison).
    ...
    if hq is not None:
        cap = _train_cap(hq)
        # G6-B EMULATOR: a MODERATE standing army (enough to defend the cluster) -- kept small
        # enough that army upkeep does not starve the HQ climb (real B funded both from a strong
        # economy). Economy + climb take priority (planned first); army is the leftover.
        want_spare = max(2, len(my_bases) // 2)
