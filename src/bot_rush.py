...
DEBUG = False
RUSH_N = 4          # rush stack size (the marching column, excluding the 1 base worker)
GOLD_BUF = 60      # keep this much for move orders so the rush never strands itself broke
...
def _impl(S, M, nav, turn, a):
    ...
    # ECONOMY: claim ONE near stronghold for income so the rush stays solvent (build it when a worker
    # stands on it). The base worker funds the column's move orders + upkeep.
    if BOT.base_reg < 0:
        BOT.base_reg = BOT.claim_order[0] if BOT.claim_order else -1
    worker = None
    if BOT.base_reg >= 0:
        worker = next((w for w in my if w.region == BOT.base_reg), None)
        if worker is not None and S.find_building(BOT.base_reg) is None and BOT.base_reg not in enemy_at:
            if S.gold - spent - BASE_LEVELS[1].cost >= 0:
                spent += BASE_LEVELS[1].cost; a.upgrades.append(BOT.base_reg)
        # send a worker to the base if none is there/incoming and no base yet
        if S.find_building(BOT.base_reg) is None and worker is None and not any(
                w.state is WState.MOVING and w.target == BOT.base_reg for w in my):
            cand = next((w for w in my if w.state is WState.STATIONARY and w.region == M.my_hq), None)
            if cand is not None:
                mv(cand, BOT.base_reg)

    # RUSH: accumulate the column at home, then march it as ONE cohesive stack at the enemy HQ
    # (stack advances a hop together; stragglers/reinforcements converge first -- a real all-in).
    column = [w for w in my if not (w is worker or (worker is None and w.region == BOT.base_reg))]
    column = [w for w in column if not (w.state is WState.MOVING and w.target == BOT.base_reg)]
    stat = [w for w in column if w.state is WState.STATIONARY]
    if len(column) >= RUSH_N:
        BOT.marching = True
    if BOT.marching and column:
        ...
        if stack == target:
            ...
            for w in at_stack:
                if nh >= 0: mv(w, nh)            # whole stack advances one hop together
            for w in stat:
                if w.region != stack: mv(w, stack)
        else:
            for w in stat:
        ...
        for w in stat:                              # gather at home until the column is RUSH_N strong
            if w.region != M.my_hq: mv(w, M.my_hq)

    # TRAIN reinforcements for the column, but never below the move buffer.
    if hq:
